...
part2 :: proc() {
  lines := strings.split_lines(INPUT)
  almanac_seeds, _ := utils.parse_int_list(strings.split(lines[0], ": ")[1])

  seed_pairs := [dynamic][2]int{}
  for i := 0; i < len(almanac_seeds); i += 2 {
    append(&seed_pairs, [2]int{ almanac_seeds[i], almanac_seeds[i+1]})
  }
